...
We could even redo the bootstrapping of all the exponential fit parameters, even though it would take a long time. By default, the code uses 75% of available cores.
...
The above two chunks of code have been set to not evaluate, but they can be run manually in the notebook.
...
This section is meant to generate bits and pieces to put in the manuscript.

## Process descriptors

Below we generate a table with descriptors of processes: learning rates and asymptotes training trials and no-cursor reaches (and aiming responses for the aiming condition). Mostly during learning, but also the washout phases in experiment 1. In each cell, the first value is from a single fit on the group average data. The second value is the 50th percentile from 5k fits on a bootstrapped sample. The 2 values in parentheses are the 2.5th percentile and the 97.5th percentile respectively, or the lower and upper bounds of the 95% confidence interval.

```{r}
knitr::kable(behaviorDescriptors(), row.names=TRUE)
```
