...
### Exponential fits on initial rotation
...
Since only the asymptote shows an effect and the rate of changes shows support _for_ the null hypothesis, we look at between-group comparisons for successive rotation sizes on asymptote only:

```{r}
expFitGroupBayesianTtest(exp=1, type='reaches', mode='learning', lambda=FALSE)
```

The asymptotes in reach-training trials get successively larger with larger rotations. No surprise there.
...
So for the two larger rotations, the asymptote of no-cursors is equivalent, but between 15 and 30 degrees and 30 and 45 degrees, the asymptote does increase. This is what it looks like in the figure as well.

### Exponential fits on washout phase
...
We look at between-group comparisons for successive rotation sizes, again only for asymptote:

```{r}
expFitGroupBayesianTtest(exp=1, type='reaches', mode='washout', lambda=FALSE)
```
...
No-cursors are not different between the groups in either parameter fit to the washout data, and for rate of change there is even support _for_ the null hypothesis.

In the figure it does look like the 15 deg group has a lower N0 than the other groups. More or less like in reach training. So, despite a lack of an effect on the starting value (N0 is not the asymptote in this case) in washouts for no-cursors, we compare the starting point (asymptote / N0) between the 15 deg group and the three other groups:
...
The evidence goes toward some small amount of re-aiming in the 15 degree condition, although this is largely driven by 2 participants and it does not reach threshold (BF=3). There is some re-aiming happening in the 30 degree condition (3 degrees on average), but still close to 0. In the other two conditions, it is much more clear that almost all participants engage in some amount of re-aiming. This can both be seen in Fig 2E and is shown by the statistics above.
...
1. Rates of change do not increases between pairs of successive rotation sizes. This goes for both reach training trials and no-cursor trials.
...
3. In washout, the overall tests indicate effects of rotation size on the starting point of reach training trials - and no effect of rotation size on washout no-cursors. The 15 degree group has a lower starting point for reach trials than the other groups.

4. Except for the lowest rotation of 15 degrees, there is some amount of re-aiming in all groups (and in the 30 degree condition it is very low). Re-aiming strategies are not different (data doesn't provide evidence one way or another) between 15 and 30 degrees, and between 45 and 60 degrees. The latter perhaps due to large variability and multi-modal distributions.

5. The amount of explicit strategies is barely informative about the amount of implicit adaptation, and only in the 60 degree group.
...
It _looks_ like the implicit time courses develop at different speeds in the figure, but there really is no effect. The data even supports the null hypothesis (equivalence).
...
Both groups are different from control, and we can see in the table that the terminal group and cursor-jump group have more than twice the amount of explicit strategy as compared to the control group.
...
2. This did not go hand-in-hand with any changes in implicit adaptation: both the asymptotic levels and the initial changes in implicit adaptation were equivalent in all conditions. Similarly the asymptotes and rates of change were equivalent for reach-training trials.

3. There was no relationship between levels of implicit adaptation and levels of explicit adaptation in any of the conditions.

In this experiment we can see that some manipulations (types of feedback) may work very well to increase explicit strategies, but this is not necessarily accompanied by any changes in the level or speed of implicit adaptation (or overall adaptation). This already shows that explicit and implicit adaptation can be independent processes.
...
This also does not indicate any differences. The evidence is unclear for asymptotes, but supports the null hypothesis for rates of change.
...
It appears that in the terminal feedback without any kind of delay, the re-aiming responses are larger than in all other groups. That is: added delays do not seem to make learning more explicit in this data set, but _less_ explicit. This is unexpected.
...
1. Terminal feedback did increase explicit adaptation relative to control. Counter to previous papers, adding delays before (or after) the terminal feedback decreased explicit re-aiming (instead of the expected increase). We don't attach much meaning to this finding here, but it shows that delays do not automatically make adaptation more explicit.

2. There were no effects on overall learning or implicit adaptation. (Except for no-cursor asymptotes, the data supported the null-hypothesis.)

3. Implicit and explicit adaptation varied in both new conditions, but independently from each other.
...
1. Adding continuous aiming increases explicit strategies. It does not affect overall learning, or implicit adaptation.
